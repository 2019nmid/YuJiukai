...
function xs(){
    //验证界面消失，然后登录账号（？？？不太清楚怎么写）
}

(function (window, document) {
    var SliderBar = function (targetDom, options) {
        // 判断是用函数创建的还是用new创建的。这样就可以通过MaskShare("dom") 或 new MaskShare("dom")来使用这个插件了  
        ...
        if (dataList.length > 0) {
            var html = "<div class='slide-box'><div class='slide-img-block'>" +
                ...
                "<div class='slide-title' id='slideHint'> 滑动左边滑块完成上面拼图</div></div></div>";
    ...
    SliderBar.prototype = {
        ...
        mousedown: function (_this, e) {
            ...
            function mousemove(event) {
                ...
                } else if (_this.x >= 0 && _this.x <= 220) {
                    _this.slideBtn.style.left = _this.x + "px";
                    _this.slideBlock.style.left = _this.x + "px";
                } else {
                    _this.slideBtn.style.left = "220px";
                    _this.slideBlock.style.left = "220px";
                }
            ...
            function mouseup() {
                ...
                if(_this.resultX > (left - 2) && _this.resultX < (left + 2)){
                    ...
                    _this.slideType.innerHTML = _this.timer + "s"+"的速度超过84%的用户";
                    setTimeout(function(){
                    ...
                    _this.slideType.innerHTML = "拖动滑块将悬浮图像正确拼合";
